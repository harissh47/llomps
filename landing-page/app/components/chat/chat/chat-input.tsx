  ...
  const handleSend = () => {
    if (onSend) {
      if (files.find(item => item.type === TransferMethod.local_file && !item.fileId)) {
        notify({ type: 'info', message: 'Please wait for the image to upload' })
        return
      }
      if (!query || !query.trim()) {
        notify({ type: 'info', message: 'Request text is required.' })
        return
      }
  ...
  const handleVoiceInputShow = () => {
    if ('webkitSpeechRecognition' in window) {
      (Recorder as any).getPermission().then(() => {
        ...
        logError('Microphone not authorized')
      })
    }
    else {
      logError('Browser not supported')
    }
  }
...
>>>>>>> origin/rupa
          ...
          `}
        // className={`
        //   p-[5.5px] max-h-[150px] bg-white dark:bg-[#3F3F3F] border-[1.5px] border-gray-200 dark:border-gray-700 rounded-xl overflow-y-auto flex items-center
        //   ${isDragActive && 'border-primary-600'} mb-2
        //   `}
              ...
              : (
                <TooltipPlus
                  popupContent={
                    <div>
                      <div>Send Enter</div>
                      <div>Line break Shift Enter</div>
                    </div>
                  }
                >
                  {sendBtn}

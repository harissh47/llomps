...
export const useCheckPromptVariables = () => {
  ...
  const checkPromptVariables = useCallback((promptVariablesConfig: {
    ...
    if (hasEmptyInput) {
      notify({ type: 'error', message: `${hasEmptyInput} value can not be empty` })
      return false
    }
  }, [notify])
...
export const useChat = (
      ...
      },
      {
        ...
        onMessageEnd: (messageEnd) => {
          ...
          const newListWithAnswer = produce(
            ...
            (draft) => {
              ...
              if (!draft[draft.length - 1].id.startsWith('action'))
                draft.push({ ...responseItem })

              draft.splice(draft.length - 1, 0, { ...responseItem })
            })
          // handleUpdateChatList(newListWithAnswer)
        ...
        onWorkflowFinished: ({ data }) => {
          ...
          handleUpdateChatList(produce(chatListRef.current, (draft) => {
            ...
            if (data.outputs.action_questions)
              responseItem.suggestedQuestions = data.outputs.action_questions
          ...
          if (data.outputs.action_type) {
            const action_type = data.outputs.action_type
            const form_values: FormValues[] = []
            let custom_forms = ''
            let action_mandatory = false

            if (data.outputs.action_mandatory)
              action_mandatory = true

            if (action_type === 'Form' && data.outputs.form_values) {
              for (let i = 0; i < data.outputs.form_values.length; i++) {
                const element = data.outputs.form_values[i]
            ...
            else if (action_type === 'Custom Form' && data.outputs.custom_forms) {
              custom_forms = data.outputs.custom_forms
            }

            const actionChatItem: ChatItem = {
              id: `action-${Date.now()}`,
              content: '',
              isAnswer: false,
              isAction: true,
            ...
            const newList = [...chatListRef.current, actionChatItem]
            handleUpdateChatList(newList)
          }
        },
  ...
  const handleActionFormInput = useCallback((actionItem: ActionItem) => {
    handleUpdateChatList(produce(chatListRef.current, (draft) => {
      // const currentIndex = draft.findIndex(item => item.id.startsWith('action')) || (draft.length - 1)
      const currentIndex = draft.length - 1
      draft[currentIndex].actions = actionItem
    }))
  }, [handleUpdateChatList])

...
const Item: FC<ItemProps> = ({
  ...
  return (
    <div
      ...
      // `}
      // className={`
      //   flex mb-0.5 last-of-type:mb-0 py-1.5 pl-3 pr-1.5 text-sm font-medium text-gray-100
      //   rounded-lg cursor-pointer hover:bg-gray-50 hover:text-black group
      //   ${currentConversationId === item.id && 'text-primary-600 bg-primary-50'}
      //   `}
      className={`
        flex mb-0.5 last-of-type:mb-0 py-1.5 pl-3 pr-1.5 text-sm font-medium text-gray-700
    ...
    >
      ...
      {/* <MessageDotsCircle className={`shrink-0 mt-1 mr-2 w-4 h-4 text-gray-400 ${currentConversationId === item.id && 'text-[#bed730]'}`} /> */}
      <MessageDotsCircle className={`shrink-0 mt-1 mr-2 w-4 h-4 text-gray-400 ${currentConversationId === item.id && 'text-[#bed730]'}`} />

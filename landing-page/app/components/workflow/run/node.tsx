...
const NodePanel: FC<Props> = ({
    ...
    return (
        <div className={cn('px-4 py-1', className, hideInfo && '!p-0')}>
            <div className={cn('group transition-all bg-white border border-gray-100 rounded-2xl shadow-xs hover:shadow-md', hideInfo && '!rounded-lg')}>                <div
                    className={cn(
                    ...
                    <div className='pb-2'>
                                    ...
                                    {justShowIterationNavArrow
                                        ...
                                        : (
                                            // <div className='flex items-center space-x-1 text-[#155EEF]'>
                                                <div className='flex items-center space-x-1 text-[#8AB40A]'>
                                                <div className='text-[13px] font-normal '>{'View details'}</div>
                        ...
                        {nodeInfo.inputs && (
                            <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
                                <CodeEditor
                                    readOnly
                                    title={<div>{'Input'.toLocaleUpperCase()}</div>}
                        ...
                        {nodeInfo.process_data && (
                            <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
                                <CodeEditor
                                    readOnly
                                    title={<div>{'Process Data'.toLocaleUpperCase()}</div>}
                                    language={CodeLanguage.json}
                        ...
                        {nodeInfo.outputs && (
                            <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
                                <CodeEditor
                                    readOnly
                                    title={<div>{'Output'.toLocaleUpperCase()}</div>}
                                    language={CodeLanguage.json}

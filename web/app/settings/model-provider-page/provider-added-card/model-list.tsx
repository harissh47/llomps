...
const ModelList: FC<ModelListProps> = ({
  ...
  return (
    <div className='px-2 pb-2 rounded-b-xl '>
      <div className='py-1 bg-white dark:bg-[#3f3f3f] rounded-lg'>
        <div className='flex items-center pl-1 pr-[3px]'>
          <span className='group shrink-0 flex items-center mr-2'>
            <span className='group-hover:hidden pl-1 pr-1.5 h-6 leading-6 text-xs font-medium text-gray-500 dark:text-[#fcfcfc]'>
              {t('common.modelProvider.modelsNum', { num: models.length })}
            </span>
            <span
              className='hidden group-hover:inline-flex items-center pl-1 pr-1.5 h-6 text-xs font-medium text-gray-500 dark:text-[#6B7280] bg-gray-50 dark:bg-zinc-800 cursor-pointer rounded-lg'
              onClick={() => onCollapse()}
            >

...
const DevelopMain = ({ appId }: IDevelopMainProps) => {
  ...
  return (
    <div className='relative flex flex-col h-full overflow-hidden'>
      ...
      <div className={`flex items-center justify-between flex-shrink-0 px-6 border-b border-solid py-2 border-b-gray-100 ${getDarkThemeClasses('borderb')} !border-solid`}>
        ...
        <div className='flex items-center flex-wrap gap-y-1'>
          ...
          <div className={`flex items-center h-9 px-3 rounded-lg 
                ...
                ${appDetail.enable_api ? `text-green-500 bg-green-50 ${getDarkThemeClasses('sub_text11')} ${getDarkThemeClasses('background3')}` : `text-yellow-500 bg-yellow-50 ${getDarkThemeClasses('sub_text10')} ${getDarkThemeClasses('background3')}`} 
                `}>
            <div className='mr-1'>{t('appApi.status')}</div>
            <div className='font-semibold'>{appDetail.enable_api ? `${t('appApi.ok')}` : `${t('appApi.disabled')}`}</div>
          </div>
          <SecretKeyButton className={`flex-shrink-0 ${getDarkThemeClasses('border')} ${getDarkThemeClasses('hovershadow')}`} appId={appId} />
        </div>
      </div>

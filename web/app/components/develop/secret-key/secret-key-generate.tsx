...
const SecretKeyGenerateModal = ({
  ...
  return (
    <Modal isShow={isShow} onClose={onClose} title={`${t('appApi.apiKeyModal.apiSecretKey')}`} className={`px-8 ${className}`}>
      ...
      <p className='mt-1 text-[13px] text-gray-500 dark:text-[#A1A2B6] font-normal leading-5'>{t('appApi.apiKeyModal.generateTips')}</p>
      <div className='my-4'>
        <InputCopy className='w-full' value={newKey?.token} />
      </div>
      <div className='flex justify-end my-4'>
        <Button type='default' className={`flex-shrink-0 dark:border-[#5F5F5F] dark:hover:bg-transparent dark:hover:shadow-[#5F5F5F] ${s.w64}`} onClick={onClose}>
          <span className='text-xs font-medium text-gray-800 dark:text-white'>{t('appApi.actionMsg.ok')}</span>
        </Button>
      </div>

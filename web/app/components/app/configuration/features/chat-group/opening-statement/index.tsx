  ...
  const headerRight = !readonly ? (
    isFocus ? (
      <div className='flex items-center space-x-1'>
        <div className='px-3 leading-[18px] text-xs font-medium text-gray-700 dark:text-white cursor-pointer' onClick={handleCancel}>{t('common.operation.cancel')}</div>
      ...
      <div>
        ...
        >
          {tempSuggestedQuestions.map((question, index) => {
            return (
              <div className='group relative rounded-lg border border-gray-200 dark:border-[#5f5f5f] flex items-center pl-2.5 hover:border-gray-300 dark:hover:border-[#5b5f5f] hover:bg-white dark:hover:bg-zinc-800' key={index}>
                ...
                <input
                  ...
                  className={'w-full overflow-x-auto pl-1.5 pr-8 text-sm leading-9 text-gray-900 dark:text-white border-0 grow h-9 bg-transparent focus:outline-none cursor-pointer rounded-lg'}
                />

                <div
                  className='block absolute top-1/2 translate-y-[-50%] right-1.5 p-1 rounded-md cursor-pointer hover:bg-[#FEE4E2] dark:hover:bg-[#3F3F3F] hover:text-[#D92D20]'
                  onClick={() => {
        ...
        {tempSuggestedQuestions.length < MAX_QUESTION_NUM && (
          <div
            ...
            className='mt-1 flex items-center h-9 px-3 gap-2 rounded-lg cursor-pointer text-gray-400 dark:text-white  bg-gray-100 dark:bg-[#333333] hover:bg-gray-200 dark:hover:bg-zinc-800'>
...
>>>>>>> origin/rupa
        {(hasValue || (!hasValue && isFocus)) ? (
              ...
              ? (
                <div>
                  <textarea
                    ...
                    className="w-full px-0 text-sm  border-0 bg-transparent dark:!text-[#FCFCFC] focus:outline-none "
          ...
          <div className='pt-2 pb-1 text-xs text-gray-500 dark:text-[#A1A6B2]'>{t('appDebug.openingStatement.noDataPlaceHolder')}</div>
        )}

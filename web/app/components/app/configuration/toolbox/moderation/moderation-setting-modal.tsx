...
const ModerationSettingModal: FC<ModerationSettingModalProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className='py-2'>
        <div className='leading-9 text-sm font-medium text-gray-900 dark:text-[#eeeeee]'>
        ...
        <div className='grid gap-2.5 grid-cols-3'>
          {
            providers.map(provider => (
              <div
                ...
                // `}
                className={`
                  flex items-center px-3 py-2 rounded-lg text-sm text-gray-900 dark:text-white cursor-pointer
                  ${localeData.type === provider.key ? 'bg-white dark:bg-[#3e3e3e] border-[1.5px] border-primary-400 shadow-sm' : 'border border-gray-100 dark:border-[#5f5f5f] bg-gray-25 dark:bg-[#2c2c2c]'}
                  ${localeData.type === 'openai_moderation' && provider.key === 'openai_moderation' && !openaiProviderConfiged && 'opacity-50'}
                `}
              ...
              >
                <div className={`
                  ...
                  ${localeData.type === provider.key ? 'border-[5px] border-primary-600' : 'border border-gray-300 dark:border-[#5f5f5f] '}`} />
                {provider.name}
              </div>
            ))
          }
      ...
      </div>
      {
        localeData.type === 'keywords' && (
          <div className='py-2'>
            ...
            <div className='mb-1 text-sm font-medium text-gray-900 dark:text-white'>{t('appDebug.feature.moderation.modal.provider.keywords')}</div>

            {/* <div className='mb-2 text-xs text-gray-500'>{t('appDebug.feature.moderation.modal.keywords.tip')}</div> */}
            <div className='mb-2 text-xs text-gray-500 dark:text-white'>{t('appDebug.feature.moderation.modal.keywords.tip')}</div>

            {/* <div className='relative px-3 py-2 h-[88px] bg-gray-100  rounded-lg'> */}
            <div className='relative px-3 py-2 h-[88px] bg-gray-100 dark:bg-[#2c2c2c] rounded-lg'>

              <textarea
                value={localeData.config?.keywords || ''}
                onChange={handleDataKeywordsChange}
                className='block w-full h-full bg-transparent text-sm outline-none appearance-none resize-none dark:text-white'
                placeholder={t('appDebug.feature.moderation.modal.keywords.placeholder') || ''}
              />
              <div className='absolute bottom-2 right-2 flex items-center px-1 h-5 rounded-md bg-gray-50 dark:bg-[#5f5f5f] text-xs font-medium text-gray-300 dark:text-[#a1a6b2 ]'>
                <span>{(localeData.config?.keywords || '').split('\n').filter(Boolean).length}</span>/<span className='text-gray-500 dark:text-white '>100 {t('appDebug.feature.moderation.modal.keywords.line')}</span>
              </div>
            </div>
          </div>
        )
        ...
        localeData.type === 'api' && (
          <div className='py-2'>
            <div className='flex items-center justify-between h-9'>
              <div className='text-sm font-medium text-gray-900 dark:text-white'>{t('common.apiBasedExtension.selector.title')}</div>
              <a

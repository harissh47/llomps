...
const PromptValuePanel: FC<IPromptValuePanelProps> = ({
  ...
  return (
    ...
    //   boxShadow: '0px 4px 8px -2px rgba(16, 24, 40, 0.1), 0px 2px 4px -2px rgba(16, 24, 40, 0.06)',
      <div className="pb-3 border border-gray-200 dark:border-[#5f5f5f] bg-white rounded-xl dark:bg-[#1A1A1A] dark:text-white" style={{
        boxShadow: '0px 4px 8px -2px rgba(16, 24, 40, 0.1), 0px 2px 4px -2px rgba(16, 24, 40, 0.06)',
    }}>
      {/* <div className={'mt-3 px-4 bg-white'}> */}
      <div className={'mt-3 px-4 bg-white dark:bg-[#1A1A1A] dark:text-white'}>
        <div className={
          ...
          <div className='flex items-center space-x-1 cursor-pointer' onClick={() => setUserInputFieldCollapse(!userInputFieldCollapse)}>
            ...
            <div className='text-xs font-medium text-gray-800 uppercase dark:text-white'>{t('appDebug.inputs.userInputField')}</div>
          </div>
          {appType === AppType.completion && promptVariables.length > 0 && !userInputFieldCollapse && (
            ...
            <div className="mt-1 text-xs leading-normal text-gray-500 dark:text-[#FCFCFC]">{t('appDebug.inputs.completionVarTip')}</div>
          )}
        </div>
        {!userInputFieldCollapse && (
          <>
            {
              promptVariables.length > 0
                  ...
                  <div className="space-y-3 ">
                    {promptVariables.map(({ key, name, type, options, max_length, required }) => (
                      <div key={key} className="xl:flex justify-between">
                        ...
                        <div className="mr-1 py-2 shrink-0 w-[120px] text-sm text-gray-900 dark:text-[#FCFCFC]">{name || key}</div>
                        {type === 'select' && (
                          <Select
                            className='w-full'
                        ...
                        {type === 'string' && (
                          <input
                            className="w-full px-3 text-sm leading-9 text-gray-900 dark:text-[#FCFCFC] border-0 rounded-lg grow h-9 bg-gray-50 dark:bg-[#3F3F3F] focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200 dark:focus:ring-[#5F5F5F]"
                        ...
                        {type === 'paragraph' && (
                          <textarea
                            ...
                            className="w-full px-3 text-sm leading-9 text-gray-900 dark:text-[#FCFCFC] border-0 rounded-lg grow h-[120px] bg-gray-50 dark:bg-[#3F3F3F] focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200 dark:focus:ring-[#5f5f5f]"
                            placeholder={`${name}${!required ? `(${t('appDebug.variableTable.optional')})` : ''}`}
                            value={inputs[key] ? `${inputs[key]}` : ''}
                            onChange={(e) => { handleInputValueChange(key, e.target.value) }}
                          />
                ...
                : (
                  ...
                  <div className='text-xs text-gray-500 dark:text-white'>{t('appDebug.inputs.noVar')}</div>
                )
            }
            {

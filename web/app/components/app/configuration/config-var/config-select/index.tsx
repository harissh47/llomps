  ...
  return (
    <div>
        ...
        <div className='mb-1'>
          ...
          >
            {options.map((o, index) => (
              ...
              <div className={`relative flex items-center rounded-lg border border-[#EAECF0] dark:border-[#5F5F5F] pl-2 cursor-pointer hover:shadow-[0px_1px_2px_rgba(16,24,40,0.05)] group`}
               key={index}
              >
                ...
                <input
                  ...
                  className={'w-full pl-1.5 pr-8 text-sm leading-9 text-gray-900 dark:text-[#FCFCFC] border-0 grow h-9 bg-transparent focus:outline-none cursor-pointer'}
                />
                <RemoveIcon
                  className={`${s.deleteBtn} absolute top-1/2 translate-y-[-50%] right-1.5 items-center justify-center w-6 h-6 rounded-md cursor-pointer hover:bg-[#FEE4E2] dark:hover:bg-zinc-800`}
                  onClick={() => {
      ...
      <div
        ...
        className='flex items-center h-9 px-3 gap-2 rounded-lg cursor-pointer text-gray-400 dark:text-[#A1A6B2] bg-gray-100 dark:bg-[#2C2C2C]'>
        <PlusIcon width={16} height={16}></PlusIcon>
        {/* <div className='text-gray-500 text-[13px]'>{t('appDebug.variableConig.addOption')}</div> */}
        <div className='text-gray-500 dark:text-[#A1A6B2] text-[13px]'>{t('appDebug.variableConig.addOption')}</div>
      </div>
    </div>
  )
}

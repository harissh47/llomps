...
const SettingBuiltInTool: FC<Props> = ({
  ...
  const infoUI = (
    <div className='pt-2'>
      ...
      <div className='leading-5 text-sm font-medium text-gray-900 dark:text-white'>
        {t('tools.setBuiltInTools.toolDescription')}
      </div>
      {/* <div className='mt-1 leading-[18px] text-xs font-normal text-gray-600'> */}
      <div className='mt-1 leading-[18px] text-xs font-normal text-gray-600 dark:text-[#FCFCFC]'>
      ...
      {infoSchemas.length > 0 && (
        <div className='mt-6'>
          <div className='flex items-center mb-4 leading-[18px] text-xs font-semibold text-gray-500 uppercase'>
            ...
            <div className='grow w-0 h-px bg-[#f3f4f6] dark:bg-[#5f5f5f]'></div>
            ...
            {infoSchemas.map((item: any, index) => (
              <div key={index}>
                <div className='flex items-center space-x-2 leading-[18px]'>
                  ...
                  <div className='text-[13px] font-semibold text-gray-900 dark:text-white'>{item.label[language]}</div>
                  ...
                  <div className='text-xs font-medium text-gray-500 dark:text-[#FCFCFC]'>{item.type === 'number-input' ? t('tools.setBuiltInTools.number') : t('tools.setBuiltInTools.string')}</div>
                  {item.required && (
    ...
    <Drawer
      ...
      title={(
        <div className='flex items-center'>
          ...
          <div className='ml-2 leading-6 text-base font-semibold text-gray-900 dark:text-white'>{currTool?.label[language]}</div>
          {(hasSetting && !readonly) && (<>
            ...
            <div className='flex space-x-6'>
              ...
              > */}
              <div
                className={cn(isInfoActive ? 'text-gray-900 dark:text-white font-semibold' : 'font-normal text-gray-600 dark:text-[#A1A2B6] cursor-pointer', 'relative text-base')}
                onClick={() => setCurrType('info')}
              >
              ...
              > */}
                <div className={cn(!isInfoActive ? 'text-gray-900 dark:text-white font-semibold' : 'font-normal text-gray-600 dark:text-[#A1A2B6] cursor-pointer', 'relative text-base ')}
                onClick={() => setCurrType('setting')}
              >
          ...
          {isLoading
            ...
            : (<div className='flex flex-col h-full'>
              ...
              {!readonly && !isInfoActive && (
                ...
                <div className='mt-2 shrink-0 flex justify-end py-4 px-6  space-x-2 rounded-b-[10px] bg-gray-50 dark:bg-[#3e3e3e] border-t border-black/5'>
                  {/* <Button className='flex items-center h-8 !px-3 !text-[13px] font-medium !text-gray-700' onClick={onHide}>{t('common.operation.cancel')}</Button> */}
                  <Button className='flex items-center h-8 !px-3 !text-[13px] font-medium !text-gray-700 dark:bg-[#3e3e3e] dark:hover:bg-zinc-800 dark:border-[#5f5f5f]' onClick={onHide}>{t('common.operation.cancel')}</Button>

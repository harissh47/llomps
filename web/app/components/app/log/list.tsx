...
function DetailPanel<T extends ChatConversationFullDetailResponse | CompletionConversationFullDetailResponse>({ detail, onFeedback }: IDetailPanel<T>) {
  ...
  return (
    ...
    <div ref={ref} className='rounded-xl border-[0.5px] border-gray-200 h-full flex flex-col overflow-auto dark:bg-[#333333] dark:border-[#5f5f5f]'>

      {/* Panel Header */}
      {/* <div className='border-b border-gray-100 py-4 px-6 flex items-center justify-between'> */}
      <div className='border-b border-gray-100 dark:border-[#5f5f5f] py-4 px-6 flex items-center justify-between'>

        <div>
          <div className='text-gray-500 dark:text-[#A1A6B2] text-[10px] leading-[14px]'>{isChatMode ? t('appLog.detail.conversationId') : t('appLog.detail.time')}</div>
          {isChatMode && (
        ...
        <div className='flex items-center flex-wrap gap-y-1 justify-end'>
          ...
          <div className='w-6 h-6 rounded-lg flex items-center justify-center hover:cursor-pointer hover:bg-gray-100 dark:hover:bg-[#3F3F3F]'>
...
const ConversationList: FC<IConversationList> = ({ logs, appDetail, onRefresh }) => {
  ...
  const renderTdValue = (value: string | number | null, isEmptyStyle: boolean, isHighlight = false, annotation?: LogAnnotation) => {
    return (
      ...
      >
        <div className={cn(isEmptyStyle ? 'text-gray-400 dark:text-[#a1a2b6]' : 'text-gray-700 dark:text-white', !isHighlight ? '' : 'bg-orange-100 dark:bg-[#3f3f3f]', 'text-sm overflow-hidden text-ellipsis whitespace-nowrap')}>
          {value || '-'}
        </div>
      </Tooltip>
    )
  ...
  return (
    <div className='overflow-x-auto'>
      <table className={`w-full min-w-[440px] border-collapse border-0 text-sm mt-3 ${s.logTable}`}>
        <thead className="h-8 leading-8 border-b border-gray-200 dark:border-[#5f5f5f] text-gray-500 font-bold">
          <tr>
        ...
        <tbody className="text-gray-500 dark:text-[#aa1a2b6]">
          {logs.data.map((log: any) => {
            ...
            return <tr
              key={log.id}
              className={`border-b border-gray-200 dark:border-[#5f5f5f] h-8 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:!text-white cursor-pointer ${currentConversation?.id !== log.id ? '' : 'bg-gray-50 dark:bg-[#3f3f3f]'}`}
              onClick={() => {
                setShowDrawer(true)

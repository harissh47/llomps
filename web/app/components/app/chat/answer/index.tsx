...
const IconWrapper: FC<{ children: React.ReactNode | string }> = ({ children }) => {
  return <div className={'rounded-lg h-6 w-6 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-zinc-700 dark:bg-[#3f3f3f]'}>
    {children}
  </div>
}
        ...
        <div ref={containerRef} className={cn(s.answerWrapWrap, 'chat-answer-container')}>
          <div className={cn(s.answerWrap, 'group')}>
            <div ref={contentRef} className={`${s.answer} relative text-sm text-gray-900`}>
              <div className={'ml-2 py-3 px-4 bg-gray-100 dark:bg-[#3f3f3f] rounded-tr-2xl rounded-b-2xl'}>
              ...
              {hasAnnotation && (
                <div
                  className={cn(s.hasAnnotationBtn, 'absolute -top-3.5 -right-3.5 box-border flex items-center justify-center h-7 w-7 p-0.5 rounded-lg bg-white cursor-pointer text-[#8AB40A]')}
                  style={{ boxShadow: '0px 4px 6px -1px rgba(0, 0, 0, 0.1), 0px 2px 4px -2px rgba(0, 0, 0, 0.05)' }}
                >
              ...
              >
                ...
                {((isShowPromptLog && !isResponding) || (!item.isOpeningStatement && isShowTextToSpeech)) && (
                  <div className='hidden group-hover:flex items-center w-max h-[28px] p-0.5 rounded-lg bg-white dark:bg-[#3f3f3f] border-[0.5px] border-gray-100 dark:border-[#5f5f5f] shadow-md shrink-0'>

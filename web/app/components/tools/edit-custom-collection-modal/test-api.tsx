...
const keyClassNames = 'py-2 leading-5 text-sm font-medium text-gray-900 dark:text-white'
          ...
          <div className='pt-2 px-6 overflow-y-auto'>
            <div className='space-y-4'>
              <div>
                ...
                <div className='flex items-center h-9 justify-between px-2.5 bg-gray-100 dark:bg-[#3F3F3F] rounded-lg cursor-pointer' onClick={() => setCredentialsModalShow(true)}>
                  ...
                  <div className='text-sm font-normal text-gray-900 dark:text-[#FCFCFC]'>{t(`tools.createTool.authMethod.types.${tempCredential.auth_type}`)}</div>
                  <Settings01 className='w-4 h-4 text-gray-700 dark:text-[#6B7280] opacity-60' />
                </div>
              </div>
              ...
              <div>
                ...
                <div className='rounded-lg border border-gray-200 dark:border-[#5F5F5F] overflow-hidden'>
                  <table className='w-full leading-[18px] text-xs text-gray-700 font-normal'>
                    <thead className='text-gray-500 uppercase'>
                      {/* <tr className='border-b border-gray-200'> */}
                      <tr className='border-b border-gray-200 dark:border-[#5F5F5F]'>
                        <th className="p-2 pl-3 font-medium dark:text-[#FCFCFC]">{t('tools.test.parameters')}</th>
                        <th className="p-2 pl-3 font-medium dark:text-[#FCFCFC]">{t('tools.test.value')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {parameters.map((item, index) => (
                        <tr key={index} className='border-b last:border-0 border-gray-200'>
                          <td className="py-2 pl-3 pr-2.5 dark:text-[#A1A6B2]">
                            {item.label[language]}
                          </td>
                          <td className="">
                            <input
                              ...
                              type='text' className='px-3 h-[34px] w-full outline-none dark:bg-[#333333] focus:bg-gray-100 dark:focus:bg-[#3F3F3F] dark:text-[#A1A6B2]' ></input>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className='mt-6'>
              <div className='flex items-center space-x-3'>
                <div className='leading-[18px] text-xs font-semibold text-gray-500 dark:text-[#FCFCFC]'>{t('tools.test.testResult')}</div>
              ...
              <div className='mt-2 px-3 py-2 h-[200px] overflow-y-auto overflow-x-hidden rounded-lg bg-gray-100 dark:bg-[#3F3F3F] leading-4 text-xs font-normal text-gray-700 dark:text-[#A1A6B2]'>

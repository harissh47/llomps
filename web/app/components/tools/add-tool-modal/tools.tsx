        ...
        {list.map((tool) => {
          ...
          return (
            <Tooltip
              ...
              position='bottom'
              className='!p-0 !px-3 !py-2.5 !w-[210px] !leading-[18px] !text-xs !text-gray-700 dark:!text-white !border-[0.5px] !border-black/5 !bg-transparent !rounded-xl !shadow-lg translate-x-[108px]'
              htmlContent={(
                <div>
                  <BlockIcon
                    size='md'
                  ...
                  <div className='text-xs text-gray-700 dark:text-[#a1a2b6] leading-[18px]'>{tool.description[language]}</div>
              ...
              noArrow
            >
              <div className='group/item flex items-center w-full pl-3 pr-1 h-8 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 cursor-pointer'>
                <BlockIcon
                  className={cn('mr-2 shrink-0', needAuth && 'opacity-30')}
                  type={BlockEnum.Tool}
                  toolIcon={toolWithProvider.icon}
                />
                <div className={cn('grow text-sm text-gray-900 dark:text-[#a1a2b6] truncate', needAuth && 'opacity-30')}>{tool.label[language]}</div>
                {!needAuth && added && (
                  <div className='flex items-center gap-1 rounded-[6px] border border-gray-100 dark:border-[#5f5f5f] px-2 py-[3px] bg-white dark:bg-[#3f3f3f] text-gray-300 dark:!text-primary-600 text-xs dark:hover:shadow-[#5F5F5F] dark:hover:bg-transparent font-medium leading-[18px]'>
                    <Check className='w-3 h-3 dark:text-[#6B7280]'/>
                ...
                {!needAuth && !added && addable && (
                  <Button
                    type='default'
                    className={cn('hidden shrink-0 items-center !h-6 px-2 py-1 bg-white dark:bg-[#3f3f3f] dark:border-[#5f5f5f] text-xs font-medium leading-[18px] text-primary-600 dark:!text-primary-600 group-hover/item:flex dark:hover:shadow-[#5F5F5F] dark:hover:bg-transparent dark:hover:border-2 dark:hover:border-[#3f3f3f]')}
                ...
                {needAuth && (
                  <Button
                    ...
                    className={cn('hidden shrink-0 items-center !h-6 px-2 py-1 bg-white dark:bg-[#3F3F3F] text-xs font-medium leading-[18px] text-primary-600 dark:!text-primary-600 dark:border-[#5F5F5F] dark:hover:shadow-[#5F5F5F] dark:hover:bg-transparent group-hover/item:flex')}

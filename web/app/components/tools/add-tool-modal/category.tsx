...
const Category = ({
  ...
  return (
    <div className='mb-3'>
      ...
      <div className={cn('mb-0.5 p-1 pl-3 flex items-center cursor-pointer text-gray-700 text-sm leading-5 rounded-lg hover:bg-white  dark:hover:bg-zinc-600', value === '' && '!bg-white dark:!bg-zinc-800 !text-primary-600 font-medium')} onClick={() => onSelect('')}>
        <Apps02 className='shrink-0 w-4 h-4 mr-2' />
        {t('tools.type.all')}
      </div>
      {labelList.map(label => (
        <div key={label.name} title={label.label[language]} className={cn('mb-0.5 p-1 pl-3 flex items-center cursor-pointer text-gray-700 dark:text-white text-sm leading-5 rounded-lg hover:bg-white dark:hover:bg-zinc-600 truncate overflow-hidden', value === label.name && '!bg-white dark:!bg-zinc-800 !text-primary-600 font-medium')} onClick={() => onSelect(label.name)}>

...
const Form: FC<FormProps> = ({
  ...
  const renderField = (formSchema: CredentialFormSchema) => {
          ...
          <div className={`grid grid-cols-${options?.length} gap-3`}>
            {
              options.filter((option) => {
                ...
                <div
                  // className={`
                  //   flex items-center px-3 py-2 rounded-lg border border-gray-100 bg-gray-25 dark:bg-[#3F3F3F] cursor-pointer
                  //   ${value[variable] === option.value && 'bg-white dark:bg-[#3F3F3F]  border-[1.5px] border-primary-400 shadow-sm'}
                  //   ${disabed && '!cursor-not-allowed opacity-60'}
                  // `}
                  className={`
                    flex items-center px-3 py-2 rounded-lg border 
                    ${value[variable] === option.value ? `border-[1.5px] border-primary-400 bg-white ${getDarkThemeClasses('background3')} shadow-sm` : `border-gray-100 ${getDarkThemeClasses('border')} bg-gray-25 ${getDarkThemeClasses('background3')}`}
                    ${disabed && '!cursor-not-allowed opacity-60'}
                    cursor-pointer
                  `}
                ...
                >
                  ...
                  <div className={`text-sm text-gray-900 ${getDarkThemeClasses('sub_text1')}`}>{option.label[language] || option.label.en_US}</div>
                </div>
              ))
            }
          </div>
    ...
    if (formSchema.type === 'boolean') {
      ...
      return (
        <div key={variable} className={cn(itemClassName, 'py-3')}>
          <div className={`flex items-center justify-between py-2 text-sm text-gray-900 ${getDarkThemeClasses('sub_text1')}`}>

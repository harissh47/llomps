            ...
            configurateMethods.map((method) => {
              if (method === ConfigurationMethodEnum.predefinedModel) {
                return (
                  <Button
                    key={method}
                    className={'h-7 bg-white text-xs text-gray-700 shrink-0 dark:border-0 dark:bg-[#3f3f3f] dark:hover:bg-zinc-800'}
              ...
              return (
                <Button
                  key={method}
                  className='px-0 h-7 bg-white text-xs text-gray-700 dark:border-0 dark:bg-[#3f3f3f] dark:hover:bg-zinc-800'
                  onClick={() => onOpenModal(method)}
                >

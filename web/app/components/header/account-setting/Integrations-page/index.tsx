      ...
      <div className='mb-8'>
        <div className={titleClassName}>{t('common.integrations.connected')}</div>
        {
          integrates.map(integrate => (
            ...
            <div key={integrate.provider} className={`mb-2 flex items-center px-3 py-2 bg-gray-50 ${getDarkThemeClasses('background2')} border-[0.5px] border-gray-200 ${getDarkThemeClasses('border')} rounded-lg`}>
              <div className={classNames(`w-8 h-8 mr-3 bg-white rounded-lg border border-gray-100 ${getDarkThemeClasses('border')}`, s[`${integrate.provider}-icon`])} />
              <div className='grow'>
                ...
                <div className={`leading-[21px] text-sm font-medium text-gray-800 ${getDarkThemeClasses('text')}`}>{integrateMap[integrate.provider].name}</div>
                <div className={`leading-[18px] text-xs font-normal text-gray-500 ${getDarkThemeClasses('sub_text1')}`}>{integrateMap[integrate.provider].description}</div>
              </div>
              {
                !integrate.is_bound && (
                  <Link
                    ...
                    className={`flex items-center h-8 px-[7px] bg-white ${getDarkThemeClasses('background3')} rounded-lg border border-gray-200 ${getDarkThemeClasses('border')} text-xs font-medium text-gray-700 ${getDarkThemeClasses('sub_text1')} cursor-pointer`}
                    href={integrate.link}

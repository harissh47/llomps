...
const PricingToggle: React.FC<PricingToggleProps> = ({ isYearly, setIsYearly }) => {
    ...
    return (
        <div className="flex items-center justify-center mb-8 space-x-4">
            ...
            <span
                className={`text-lg ${isYearly ? `text-gray-500  ${getDarkThemeClasses('sub_text7')}` : monthlyBilling.labelClass}`}
            >
                {monthlyBilling.text}
            </span>
            ...
            {toggleButton.enabled && (
                <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <div className={`w-12 h-5 bg-gray-300 rounded-full peer-checked:bg-black  ${getDarkThemeClasses('peercheckedbg')} peer-focus:ring-4 peer-focus:ring-black/20  ${getDarkThemeClasses('peerfocus')} transition duration-200`}>
            ...
            <span
                className={`text-lg ${isYearly ? yearlyBilling.labelClass : `text-gray-500 ${getDarkThemeClasses('sub_text7')}`}`}
            >
                {yearlyBilling.text}
            </span>

...
const UserInput = () => {
  ...
  return (
    <div
      ...
      className={`
        relative rounded-xl border z-[1]
        ${!expanded ? 'bg-indigo-25 border-indigo-100 shadow-none' : 'bg-white dark:bg-[#3e3e3e] shadow-xs dark:shadow-[#5f5f5f] border-transparent'}
      `}
    >
      {/* <div
        className={`
      ...
      > */}
       <div
        className={`
          flex items-center px-2 pt-4 h-[18px] text-[13px] font-semibold cursor-pointer
          ${!expanded ? 'text-indigo-800' : 'text-gray-800 dark:!text-white'}
        `}
        onClick={() => setExpanded(!expanded)}
      >
        <ChevronDown

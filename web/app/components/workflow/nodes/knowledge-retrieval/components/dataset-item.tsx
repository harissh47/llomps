...
const DatasetItem: FC<Props> = ({
  ...
  return (
    ...
    <div className='flex items-center h-10 justify-between rounded-xl px-2 bg-white dark:bg-[#3F3F3F] border border-gray-200 dark:border-[#5F5F5F] cursor-pointer group/dataset-item'>
      <div className='w-0 grow flex items-center space-x-1.5'>
        {
          payload.data_source_type === DataSourceType.NOTION
            ...
            : <div className='shrink-0 flex items-center justify-center w-6 h-6 bg-[#F5F8FF] dark:bg-[#3F3F3F] rounded-md border-[0.5px] border-[#E0EAFF] dark:border-[#5F5F5F]'>
              <Folder className='w-4 h-4 text-[#8AB40A]' />
            </div>
        }
        {/* <div className='w-0 grow text-[13px] font-normal text-gray-800 truncate'>{payload.name}</div> */}
        <div className='w-0 grow text-[13px] font-normal text-gray-800 dark:text-[#FCFCFC] truncate'>{payload.name}</div>
      </div>
      {!readonly && (

...
const AddBlock = ({
  ...
  const renderTriggerElement = useCallback((open: boolean) => {
    return (
      <div className={cn(
        'relative inline-flex items-center px-3 h-8 rounded-lg border-[0.5px] border-gray-50 dark:border-[#5f5f5f] bg-white dark:bg-[#3f3f3f] dark:text-white shadow-xs cursor-pointer hover:bg-gray-200 text-[13px] font-medium text-gray-700 dark:hover:bg-zinc-800',
        `${nodesReadOnly && '!cursor-not-allowed opacity-50'}`,
        open && '!bg-gray-50 dark:!bg-[#3F3F3F]',
      )}>

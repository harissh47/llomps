...
const TagInput: FC<TagInputProps> = ({
  ...
  return (
    ...
    <div className={cn('flex flex-wrap', !isInWorkflow && 'min-w-[200px]', isSpecialMode ? 'bg-gray-100 dark:bg-[#2C2C2C] rounded-lg pb-1 pl-1' : '')}>
      {
      ...
      }
      {
        !disableAdd && (
          <AutosizeInput
            inputClassName={cn('outline-none appearance-none placeholder:text-gray-300 dark:text-white caret-primary-600 hover:placeholder:text-gray-400', isSpecialMode ? 'bg-transparent' : '')}
            className={cn(
              !isInWorkflow && 'max-w-[300px]',
              isInWorkflow && 'max-w-[146px]',
              `
              mt-1 py-1 rounded-lg border border-transparent text-sm  overflow-hidden
              ${focused && 'px-2 border !border-dashed !border-gray-200 dark:!border-[#5f5f5f]'}
            `)}

      ...
      <div className={classNames('relative')}>
        <div className='group text-gray-800'>
          ...
          <Combobox.Button className={`absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none group-hover:bg-gray-200 ${getDarkThemeClasses('grouphover')}`} onClick={
            ...
            {open ? <ChevronUpIcon className={`h-5 w-5 ${getDarkThemeClasses('svg')}`} /> : <ChevronDownIcon className={`h-5 w-5 ${getDarkThemeClasses('svg')}`} />}

          </Combobox.Button>
        </div>

        {filteredItems.length > 0 && (
          <Combobox.Options className={`absolute z-10 mt-1 px-1 max-h-60 w-full overflow-auto rounded-md bg-white ${getDarkThemeClasses('background1')} py-1 text-base shadow-lg border-gray-200 ${getDarkThemeClasses('border')} border-[0.5px] focus:outline-none sm:text-sm ${overlayClassName}`}>
            ...
            <Listbox.Options className={`absolute z-10 mt-1 px-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg border-gray-200 ${getDarkThemeClasses('border')} border-[0.5px] focus:outline-none sm:text-sm ${getDarkThemeClasses('background1')}`}>
              {items.map((item: Item) => (
                <Listbox.Option
                  ...
                  `relative cursor-pointer select-none py-2 pl-3 pr-9 rounded-lg hover:bg-gray-100 ${getDarkThemeClasses('hover')} text-gray-700 ${getDarkThemeClasses('text')} ${getDarkThemeClasses('hover')}  ${active ? ` ${getDarkThemeClasses('text')} ${getDarkThemeClasses('hover')}` : ''
    ...
    >
      <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)} className='w-full'>
        <div
          ...
          // `}
          className={`
            flex items-center justify-between px-2.5 h-9 rounded-lg border-0 bg-[#202020] text-sm cursor-pointer 
          `}
      ...
      <PortalToFollowElemContent className={`z-20 ${popupClassName}`}>
        <div
          ...
          className='px-1 py-1 max-h-60 overflow-auto rounded-md bg-[#3f3e3e] text-base shadow-lg focus:outline-none sm:text-sm'
        >
          {items.map((item: Item) => (
            <div
              key={item.value}
              ...
              // `}
              className={`
                flex items-center justify-between px-2.5 h-9 cursor-pointer rounded-lg hover:bg-gray-100 text-gray-200 hover:text-black
                ${item.value === value && 'bg-black'}
              `}
              title={item.name}
              onClick={() => {
                onSelect(item)

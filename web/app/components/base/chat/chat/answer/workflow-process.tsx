  ...
  return (
    <div
      ...
      className={cn(
        'mb-2 rounded-xl border-[0.5px] border-black/8',
        collapse ? 'py-[7px]' : hideInfo ? 'pt-2 pb-1' : 'py-2',
        collapse && (!grayBg ? 'bg-white dark:bg-[#333333]' : 'bg-gray-50 dark:bg-[#383838]'),
        hideInfo ? 'mx-[-8px] px-1' : 'w-full px-3',
      )}
      style={{
        background,
    ...
    >
      ...
      >
        ...
        <div className='grow text-xs font-medium text-gray-700 dark:text-[#A1A6B2]'>
          {t('workflow.common.workflowProcess')}
        </div>
        <ChevronRight className={`'ml-1 w-3 h-3 text-gray-500 dark:text-[#6B7280]' ${collapse ? '' : 'rotate-90'}`} />
      </div>
      {
        !collapse && (

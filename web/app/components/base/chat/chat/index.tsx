...
const Chat: FC<ChatProps> = ({
  ...
  const { appData, currentConversationId } = useChatWithHistoryContext();
  ...
  const handleActionInputChange = useCallback(
    (list: ActionItem, val: FormValues[]) => {
  ...
  useEffect(() => {
    if (chatList[chatList.length - 1]?.isAction || false) {
      ...
      if (
        chatList[chatList.length - 1].actions?.action_mandatory &&
        !modalOpened.includes(actionId)
      )
  ...
  const handleUpdateMemory = useCallback(
    async (formValue: any, app_id: string, conversation_id: string) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full flex">
        ...
        <div className="relative h-full flex-grow bg-white dark:bg-[#333333]">
          <div
            ref={chatContainerRef}
            // className={classNames('relative h-full overflow-y-auto ', chatContainerClassName)}
            // className={classNames('relative h-full overflow-y-auto dark:bg-[#333333] ', chatContainerClassName)}
            className={classNames(
              "relative h-[90vh] overflow-y-auto dark:bg-[#3e3e3e] ",
              chatContainerClassName
            )}
          >
            {chatNode}
            {config?.show_carousel === true &&
            chatList.length === 1 &&
            chatList[0].isOpeningStatement! === true ? (
              <Carousel
                onCardClick={handleCarouselClick}
                carouselImages={config.carousel_images}
              />
            ) : config?.show_openingcontainer === true &&
              chatList.length === 1 &&
              chatList[0].isOpeningStatement! === true ? (
              <OpeningContainer
                onQuestionClick={handleCarouselClick}
              ...
              >
                {chatList.map((item, index) => {
                  if (item.isAnswer) {
                    const isLast =
                      item.id === chatList[chatList.length - 1]?.id;
                    return (
                      <Answer
                        key={item.id}
                        item={item}
                    ...
                    return (
                      ...
                      >
                        <Action
                          key={item.id}
                          onChange={(formValues) =>
                            handleActionInputChange(item.actions!, formValues)
                          }
                          ...
                          onUpdateMemory={
                            item.actions.memory
                              ? () => {
                                  ...
                                  for (
                                    let i = 0;
                                    i < item.actions!.form_values.length;
                                    i++
                                  ) {
                                    const element =
                                      item.actions!.form_values[i];
          ...
          {/* <div
            ...
            ref={chatFooterRef}
            style={{ zIndex: 10, backgroundColor: 'white', width: '100%' }} */}
          <div
          ...
          >
            ...
            >
              {!noStopResponding && isResponding && (
                <div className="flex justify-center mb-2 relative top-[-30px]">
                  {/* <Button className='py-0 px-3 h-7 bg-white shadow-xs' onClick={onStopResponding}> */}
                  <Button
                    className="py-0 px-3 h-7 bg-white shadow-xs dark:bg-[#3F3F3F] dark:border-[#5F5F5F] dark:hover:border-[#5F5F5F] dark:hover:bg-transparent"
                    onClick={onStopResponding}
                  >
                    <StopCircle className="mr-[5px] w-3.5 h-3.5 text-gray-500" />
                    {/* <span className='text-xs text-gray-500 font-normal'>{t('appDebug.operation.stopResponding')}</span> */}
                    <span className="text-xs text-gray-500 dark:text-[#FCFCFC] font-normal">

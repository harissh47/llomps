...
const Item: FC<ItemProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={cn(isActive ? 'text-gray-900 dark:text-white'  : 'text-gray-600 dark:text-white')}>{option.text}</div>
      {isActive && (
        // <div className='absolute bottom-0 left-0 right-0 h-0.5 bg-[#155EEF]'></div>
        <div className='absolute bottom-0 left-0 right-0 h-0.5 bg-[#8AB40A]'></div>
      )}
    </div>
  )
}
...
const TabSlider: FC<Props> = ({
  ...
  return (
    ...
    <div className={cn(className, !noBorderBottom && 'border-b border-[#5f5f5f]', 'flex  space-x-6')}>

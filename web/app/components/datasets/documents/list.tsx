...
export const StatusItem: FC<{
  ...
  className={cn(
    ...
    reverse ? 'flex-row-reverse' : '',
    scene === 'detail'
      ? 'h-8 font-medium border border-gray-200 dark:border-[#5F5F5F] inline-flex items-center rounded-lg pl-3 pr-4 mr-2'
      : ''
  )}  
  >
    <Indicator color={DOC_INDEX_STATUS_MAP[localStatus]?.color as IndicatorProps['color']} className={reverse ? 'ml-2' : 'mr-2'} />
    <span className={cn('text-gray-700 dark:text-white dark:text-[#667085] text-sm', textCls)}>{DOC_INDEX_STATUS_MAP[localStatus]?.text}</span>
    {
      errorMessage && (
        <Tooltip
  ...
  return <div className='flex items-center' onClick={e => e.stopPropagation()}>
    ...
    {embeddingAvailable && (
      <Popover
        htmlContent={
          ...
          <div className='w-full py-1 dark:bg-[#333333]'>

            {!isListScene && <>
              <div className='flex justify-between items-center mx-4 pt-2'>
                {/* <span className={cn(s.actionName, 'font-medium')}> */}
                <span className={cn('text-gray-700 dark:text-white text-sm font-medium')}>
              ...
              <div className='mx-4 pb-1 pt-0.5 text-xs text-gray-500 dark:text-[#A1A6B2]'>
            ...
            {!archived && (
              <>
                ...
                <div className="h-9 py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer" onClick={() => {
                  ...
                  <span className="text-gray-700 dark:text-[#FCFCFC] text-sm">{t('datasetDocuments.list.table.rename')}</span>
                </div>
                {/* <div className={s.actionItem} onClick={() => router.push(`/datasets/${datasetId}/documents/${detail.id}/settings`)}> */}
                <div className="h-9 py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-zinc-800  rounded-lg cursor-pointer" onClick={() => router.push(`/data-mind/${datasetId}/documents/${detail.id}/settings`)}>
                  <SettingsIcon />
                  {/* <span className={s.actionName}>{t('datasetDocuments.list.action.settings')}</span> */}
                  <span className="text-gray-700 dark:text-[#FCFCFC] text-sm">{t('datasetDocuments.list.action.settings')}</span>
                </div>
            ...
            {!archived && <div className="h-9 py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-zinc-800  rounded-lg cursor-pointer" onClick={() => onOperate('archive')}>

              <ArchiveIcon />
              {/* <span className={s.actionName}>{t('datasetDocuments.list.action.archive')}</span> */}
              <span className="text-gray-700 dark:text-[#FCFCFC] text-sm">{t('datasetDocuments.list.action.archive')}</span>

            </div>}
            {archived && (
              // <div className={s.actionItem} onClick={() => onOperate('un_archive')}>
              <div
               className="h-9 py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"
               onClick={() => onOperate('un_archive')}
              >
                <ArchiveIcon />
                {/* <span className={s.actionName}>{t('datasetDocuments.list.action.unarchive')}</span> */}
                <span className="text-gray-700 dark:text-[#FCFCFC] text-sm">{t('datasetDocuments.list.action.unarchive')}</span>
              </div>
            )}
            {/* <div className={cn(s.actionItem, s.deleteActionItem, 'group')} onClick={() => setShowModal(true)}> */}
            <div
             className="h-9 py-2 px-3 mx-1 flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer hover:bg-red-50 group"
            ...
            >
              ...
              <span className="text-gray-700 dark:text-[#FCFCFC] text-sm group-hover:text-red-500">{t('datasetDocuments.list.action.delete')}</span>
            </div>
          </div>
        }
        trigger='click'
    ...
    {showModal && <Modal isShow={showModal} onClose={() => setShowModal(false)} className={s.delModal} closable>
      <div>
        ...
        <div className="h-12 w-12 border-[0.5px] border-gray-100 dark:bg-[#3F3F3F] dark:border-[#3F3F3F] rounded-xl mb-3 flex items-center justify-center shadow-[0px_20px_24px_-4px_rgba(16,24,40,0.08),0px_8px_8px_-4px_rgba(16,24,40,0.03)] bg-[rgba(255,255,255,0.9)]">
        ...
        <div className='text-xl font-semibold text-gray-900 dark:text-white mb-1'>{t('datasetDocuments.list.delete.title')}</div>
        <div className='text-sm text-gray-500 dark:text-[#FCFCFC] mb-10'>{t('datasetDocuments.list.delete.content')}</div>
        <div className='flex gap-2 justify-end'>
          <Button className='mr-2 text-sm font-medium dark:border-0 dark:bg-[#3f3f3f] dark:hover:bg-zinc-800' onClick={() => setShowModal(false)}>{t('common.operation.cancel')}</Button>
          <Button           
            type='warning'
...
export const renderTdValue = (value: string | number | null, isEmptyStyle = false) => {
  return (
    ...
    <div className={cn(isEmptyStyle ? 'text-gray-400 dark:text-[#a1a6b2]' : 'text-gray-700 dark:text-[#a1a6b2]', s.tdValue)}>
  ...
  return (
    <div className='w-full h-full overflow-x-auto'>
      <table className={`min-w-[700px] max-w-full w-full border-collapse border-0 text-sm mt-3 ${s.documentTable}`}>
        ...
        <thead className="h-8 leading-8 border-b border-gray-200 dark:border-[#5f5f5f] text-gray-500 font-medium text-xs uppercase">

          <tr>
          {/* <td className='w-12'>#</td> */}
            <td className='w-12 dark:text-white'>#</td>
            <td>
              {/* <div className='flex'> */}
            ...
            <td className='w-24 dark:text-white'>{t('datasetDocuments.list.table.header.words')}</td>
            <td className='w-44 dark:text-white'>{t('datasetDocuments.list.table.header.hitCount')}</td>
            <td className='w-44'>
            ...
            <td className='w-40 dark:text-white'>{t('datasetDocuments.list.table.header.status')}</td>
            <td className='w-20 dark:text-white'>{t('datasetDocuments.list.table.header.action')}</td>
          </tr>
        </thead>
        <tbody className="text-gray-700">
          {localDocs.map((doc) => {
            ...
            return <tr
              ...
              className={'border-b border-gray-200 dark:border-[#5f5f5f] h-8 hover:bg-gray-50 dark:hover:bg-zinc-800 cursor-pointer'}
              onClick={() => {
                // router.push(`/datasets/${datasetId}/documents/${doc.id}`)
                router.push(`/data-mind/${datasetId}/documents/${doc.id}`)
              }}>
              <td className='text-left align-middle text-gray-500 text-xs'>{doc.position}</td>
              <td>
                <div className='group flex items-center justify-between'>
                  {/* <span className={s.tdValue}> */}
                  {/* <span className="text-sm overflow-hidden text-ellipsis dark:text-[#667085] whitespace-nowrap"> */}
                  <span className="text-sm overflow-hidden text-ellipsis dark:text-[#667085] dark:text-[#a1a6b2] whitespace-nowrap">
              ...
              <td className='text-gray-500 dark:text-[#a1a6b2] text-[13px]'>

...
const EmptyElement: FC<{ canAdd: boolean; onClick: () => void; type?: 'upload' | 'sync' }> = ({ canAdd = true, onClick, type = 'upload' }) => {
  ...
  return <div className={s.emptyWrapper}>
    ...
    <div className="bg-gray-50 dark:bg-[#3F3F3F] w-[560px] h-fit box-border px-5 py-4 rounded-2xl">
      {/* <div className={s.emptySymbolIconWrapper}> */}
      <div className="pt-6 pb-2 flex items-center font-medium text-lg leading-6 text-gray-700 dark:border-[#5F5F5F] flex justify-between items-center">
      ...
      <span className="text-gray-700 dark:text-white font-semibold inline relative -top-3 -left-1.5">
      {t('datasetDocuments.list.empty.title')}
      <ThreeDotsIcon className="inline relative -top-3 -left-1.5" />
      </span>
       <div className="mt-2 text-gray-500 dark:text-[#FCFCFC] text-sm font-normal">
...
const Documents: FC<IDocumentsProps> = ({ datasetId }) => {
  ...
  const routeToDocCreate = () => {
    ...
    router.push(`/data-mind/${datasetId}/documents/create`)
  }
...
>>>>>>> origin/rupa
      ...
      <div className='flex items-center justify-start px-3'>
        ...
        <div className='flex flex-col justify-center gap-1 px-2 pt-4'>
          {/* <h1 className={s.title}>{t('datasetDocuments.list.title')}</h1> */}
          <h1 className={cn('text-xl font-medium text-gray-900 dark:text-white')}>
                {t('datasetDocuments.list.title')}
          </h1>

          {/* <p className={s.desc}>{t('datasetDocuments.list.desc')}</p> */}
          {/* <p className={s.desc}>All files of the Knowledge are shown here, and the entire Knowledge can be linked to citations or indexed via the Chat plugin.</p> */}
          {/* <p className={s.desc}>All files of the Data Mind are shown here, and the entire Data Mind can be linked to citations or indexed via the Chat plugin.</p> */}
          <p className="text-sm font-normal text-gray-500 dark:text-[#FCFCFC]">
             All files of the Data Mind are shown here, and the entire Data Mind can be linked to citations or indexed via the Chat plugin.
         </p>

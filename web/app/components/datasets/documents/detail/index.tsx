...
export const DocumentTitle: FC<DocumentTitleProps> = ({ extension, name, iconCls, textCls, wrapperCls }) => {
  ...
  return <div className={cn('flex items-center justify-start flex-1', wrapperCls)}>
    ...
    <span className={cn('font-semibold text-lg text-gray-900 dark:text-white ml-1', textCls)}> {name || '--'}</span>
  </div>
}
...
const DocumentDetail: FC<Props> = ({ datasetId, documentId }) => {
  ...
  const backToPrev = () => {
    ...
    router.push(`/data-mind/${datasetId}/documents`)
  }
  ...
  return (
    <DocumentContext.Provider value={{ datasetId, documentId, docForm: documentDetail?.doc_form || '' }}>
      ...
      <div className='flex flex-col '>
        <div className='flex min-h-16 border-b-gray-100 dark:border-b-[#5f5f5f] border-b items-center p-4 justify-between flex-wrap gap-y-2'>
          ...
          <div className='flex items-center flex-wrap gap-y-2'>
            ...
            /> */}
            <button
             className={cn("w-8 h-8 ml-2 box-border border border-gray-200 dark:border-[#5F5F5F] rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 cursor-pointer hover:shadow-[0_1px_2px_rgba(16,24,40,0.05)]",
       ...
       />
        ...
        <div className='flex flex-row' style={{ height: 'calc(100% - 10rem)' }}>
          <div
            className={cn('h-screen flex flex-col overflow-y-scroll transition-all duration-300 pr-4', showMetadata ? 'w-2/3' : 'w-full')}>
            {isDetailLoading
              ? (<Loading type='app' />
                // : <div className={`h-full w-full flex flex-col ${embedding ? 'px-6 py-3 sm:py-12 sm:px-16' : 'pb-[30px] pt-3 px-6'}`}>
              ) : (
                // <div className={`h-full overflow-y-scroll w-2/3 flex flex-col ${embedding ? 'px-6 py-3 sm:pb-12 sm:pr-0' : 'pb-[30px] pt-3 px-6'}`}>
                <div className={`h-full w-full flex flex-col ${embedding ? 'px-6 py-3 sm:pb-12 sm:pr-0' : 'pb-[30px] pt-3 px-6'}`}>
                  {embedding ? (

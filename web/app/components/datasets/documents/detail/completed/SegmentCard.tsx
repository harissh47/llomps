  ...
  return (
    ...
    >
      <div className={s.segTitleWrapper}>
        {isDocScene
          ? <>
            <SegmentIndexTag positionId={position} className={cn('w-fit grid-col-2 group-hover:opacity-100', (isDocScene && !enabled) ? 'opacity-50' : '')} />
      ...
      {loading
        ? (
          <div className={cn(s.cardLoadingWrapper, s.cardLoadingIcon)}>
            ...
            <div
  className="h-full relative rounded-b-xl mt-4"
  style={{
    left: 'calc(-1rem - 1px)',
    width: 'calc(100% + 2rem + 2px)',
    height: 'calc(100% - 1rem + 1px)',
    background: 'linear-gradient(180deg, rgba(252, 252, 253, 0) 0%, #a1a6b2 74.15%)',
  }}
/>
          </div>
        )

    ...
    <div className='mt-6'>
      ...
      <div ref={dropRef}>
        {!file && (
          ...
          <div className={cn('flex items-center h-20 rounded-xl bg-gray-50 dark:bg-[#3f3f3f] border border-dashed border-gray-200 dark:border-[#5f5f5f] text-sm font-normal', dragging && 'bg-[#F5F8FF] border border-[#B2CCFF]')}>
        ...
        {file && (
          ...
          <div className={cn('flex items-center h-20 px-6 rounded-xl bg-gray-50 dark:bg-[#383838] border border-gray-200 dark:border-[#5f5f5f] text-sm font-normal group', 'hover:bg-[#F5F8FF] hover:border-[#B2CCFF]')}>
            ...
            <div className='flex ml-2 w-0 grow'>
              ...
              <span className='max-w-[calc(100%_-_30px)] text-ellipsis whitespace-nowrap overflow-hidden text-gray-800 dark:text-[#a1a6b2]'>{file.name.replace(/.csv$/, '')}</span>

              <span className='shrink-0 text-gray-500'>.csv</span>
            </div>
            <div className='hidden group-hover:flex items-center'>
              <Button className='!h-8 !px-3 !py-[6px] bg-white !text-[13px] !leading-[18px] text-gray-700 dark:bg-primary-600 dark:border-0' onClick={selectHandle}>{t('datasetCreation.stepOne.uploader.change')}</Button>

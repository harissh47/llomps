...
class FormMemory(db.Model):
    __tablename__ = 'form_memory'
    __table_args__ = (
        db.PrimaryKeyConstraint('conversation_id', name='form_memory_pkey'),
        db.Index('form_memory_idx', 'conversation_id', 'app_id')
    )
   
    conversation_id = db.Column(StringUUID, nullable=False)
    app_id = db.Column(StringUUID, nullable=False)
    created_at = db.Column(db.DateTime, nullable=False, server_default=db.text('CURRENT_TIMESTAMP(0)'))
    form_value = db.Column(db.JSON)

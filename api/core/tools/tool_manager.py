...
class ToolManager:
    ...
    def user_list_providers(cls, user_id: str, tenant_id: str, typ: UserToolProviderTypeLiteral) -> list[UserToolProvider]:
        ...
        disabled_providers = {
            'builtin' : ['arxiv', 'pubmed', 'aippt', 'wolframalpha', 'gaode', 'wecom', 'dingtalk', 'feishu', 'feishu_base', 'trello', 'judge0ce', 'vanna', 'devdocs', 'spark'],
        }
        ...
        if 'builtin' in filters:
            ...
            for provider in builtin_providers:
                if provider.identity.name not in disabled_providers['builtin']:
                    if provider.identity.name not in result_providers:
                        user_provider = ToolTransformService.builtin_provider_to_user_provider(
                            provider_controller=provider,
                            db_provider=find_db_builtin_provider(provider.identity.name),
                            decrypt_credentials=False
                        )
                        result_providers[f'builtin_provider.{provider.identity.name}'] = user_provider

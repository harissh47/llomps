...
class TenantService:
    ...
    @staticmethod
    def create_guest_tenant(name: str) -> Tenant:
        """Create Guest tenant"""
        tenant = Tenant(name=name)
    ...
    @staticmethod
    def create_guest_tenant_if_not_exist(account: Account):
        """Create Guest tenant if not exist"""
        available_gta = TenantAccountJoin.query.filter_by(account_id=account.id) \
            .order_by(TenantAccountJoin.id.asc()).first()
        
        if available_gta:
            return
        
        tenant = TenantService.create_guest_tenant(f"{account.name}'s Workspace")
        TenantService.create_guest_tenant_member(tenant, account, role='editor')
        account.current_tenant = tenant
        db.session.commit()
        tenant_was_created.send(tenant)
    ...
    def create_tenant_member(tenant: Tenant, account: Account, role: str = 'normal') -> TenantAccountJoin:
        ...
        return ta
    @staticmethod
    def create_guest_tenant_member(tenant: Tenant, account: Account, role: str = 'normal') -> TenantAccountJoin:
        """Create Guest tenant member"""
    
        gta = TenantAccountJoin(
            tenant_id=tenant.id,
            account_id=account.id,
            role=role
        )
...
class RegisterService:
    ...
    @classmethod
    def guestregister(cls, email, name, password: str = None, open_id: str = None, provider: str = None,
                 language: str = None, status: AccountStatus = None) -> Account:
        db.session.begin_nested()
        """Register account"""
        try:
